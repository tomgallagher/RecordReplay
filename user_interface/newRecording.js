...
function addNewRecordingEventToTable(recording, recordingEvent, table) {
    ...
    if (recordingEvent.recordingEventAction == 'Page') {
    ...
    recordingEventSelectorNode.textContent =
        recordingEvent.recordingEventCssSelectorPath || recordingEvent.recordingEventCssDomPath;
    //any text-overflow elements should have a title with the whole string
    recordingEventSelectorNode.title =
        recordingEvent.recordingEventCssSelectorPath || recordingEvent.recordingEventCssDomPath;
    ...
    recordingEventInputValueNode.textContent =
        recordingEvent.recordingEventActionType == 'selectstart'
            ? recordingEvent.recordingEventTextSelectTextContent
            : recordingEvent.recordingEventInputValue;
    //any text-overflow elements should have a title with the whole string
    recordingEventInputValueNode.title =
        recordingEvent.recordingEventActionType == 'selectstart'
            ? recordingEvent.recordingEventTextSelectTextContent
            : recordingEvent.recordingEventInputValue;
    ...
    recordingEventCreatedNode.textContent =
        recordingEvent.recordingTimeSincePrevious == 0
            ? new Date(recordingEvent.recordingEventCreated).toLocaleString()
            : `+ ${Math.ceil(recordingEvent.recordingTimeSincePrevious / 1000)} sec`;
    ...
    $('.ui.newRecordingRecordingEventsTable .showRecordingEventRow').on('click', function () {
        ...
        StorageUtils.getSingleObjectFromDatabaseTable('recordings.js', recordingKey, 'recordings')
            ...
            .then((recording) => {
                ...
                $('.ui.viewNewRecordingEvent.form input[name=recordingEventCssSelectorPath]').val(
                    recordingEvent.recordingEventCssSelectorPath
                );
                $('.ui.viewNewRecordingEvent.form input[name=recordingEventCssDomPath]').val(
                    recordingEvent.recordingEventCssDomPath
                );
                $('.ui.viewNewRecordingEvent.form input[name=recordingEventCssFinderPath]').val(
                    recordingEvent.recordingEventCssFinderPath
                );
                $('.ui.viewNewRecordingEvent.form input[name=recordingEventXPath]').val(
                    recordingEvent.recordingEventXPath
                );
                $('.ui.viewNewRecordingEvent.form input[name=recordingEventLocation]').val(
                    recordingEvent.recordingEventLocation
                );
                //then the checkbox
                recordingEvent.recordingEventIsIframe == true
                    ? $('.ui.viewNewRecordingEvent .ui.checkbox input[name=recordingEventIsIframe]').prop(
                          'checked',
                          true
                      )
                    : $('.ui.viewNewRecordingEvent .ui.checkbox input[name=recordingEventIsIframe]').prop(
...
function addStartRecordingHandler() {
    ...
    const recordingEventObservable = Rx.Observable.fromEvent(
        document.querySelector('.ui.startRecording.positive.button'),
        'click'
    )
        ...
        .do((event) => {
            ...
            ga('send', {
                hitType: 'event',
                eventCategory: 'NewRecording',
                eventAction: `Click`,
                eventLabel: 'RecordingCreationData',
            });
        })
        ...
        .flatMap((event) =>
            Rx.Observable.fromPromise(
                StorageUtils.getSingleObjectFromDatabaseTable(
                    'newRecording.js',
                    event.target.getAttribute('data-recording-id'),
                    'recordings'
                )
            )
        )
    ...
    StorageUtils.getAllObjectsInDatabaseTable('newRecording.js', 'tests')
        ...
        .then((testStorageArray) => {
            ...
            for (let test in testStorageArray) {
                ...
                newRecordingDropDownMenu.append(
                    `<div class="item" data-value=${testStorageArray[test].id}>${testStorageArray[test].testName}</div>`
                );
            }
    ...
    $('.ui.newRecordingForm.form .ui.radio.device.checkbox').checkbox({
        onChecked: function () {
            ...
            if ($(this).attr('value') == 'mobile') {
                ...
                ga('send', {
                    hitType: 'event',
                    eventCategory: 'RecordingParams',
                    eventAction: `${$(this).attr('value')}`,
                    eventLabel: 'RecordingCreationData',
                });
            } else {
    ...
    $('.ui.newRecordingForm.form').form({
        ...
        fields: {
            ...
            recordingTestStartUrl: {
                ...
                rules: [{ type: 'empty', prompt: 'Please enter test start url by selecting a test' }],
            },
        },
                ...
                .then((test) => {
                    ...
                    StorageUtils.addModelObjectToDatabaseTable('newRecording.js', newRecording, 'recordings')
                        ...
                        .then((createdRecordingId) => {
                            ...
                            $('.ui.startRecording.positive.button, .ui.stopRecording.negative.button').attr(
                                'data-recording-id',
                                createdRecordingId
                            );
