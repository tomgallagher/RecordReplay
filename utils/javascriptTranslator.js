class JavascriptTranslator {
    ...
    mouseClick = (selector, clicktype, index) =>
        `const event${index} = new MouseEvent('${clicktype}', {view: window, bubbles: true, cancelable: false}); ${this.tabIndex(
            index
        )}document.querySelector('${selector}').dispatchEvent( event${index} );`;

    recaptcha = (selector, index) =>
        `const event${index} = new MouseEvent('click', {view: window, bubbles: true, cancelable: false}); ${this.tabIndex(
            index
        )}document.querySelector('${selector}').dispatchEvent( event${index} );`;
    ...
    nonInputTyping = (selector, recordingEvent, index) => {
        ...
        if (recordingEvent.recordingEventHTMLTag != 'HTML') {
            prependForTarget = `document.querySelector('${selector}').focus({ preventScroll: false });${this.tabIndex(
                index
            )}`;
        }
        //then we just need to return the string
        return `${prependForTarget}const event${index} = new KeyboardEvent('keypress', { key: ${
            dispatchEvent.key
        }, code: ${dispatchEvent.code}, location: ${dispatchEvent.location}, repeat: ${
            dispatchEvent.autoRepeat
        }, ctrlKey: ${ctrlKey}, shiftKey: ${shiftKey}, altKey: ${altKey}, metaKey: ${metaKey}}); ${this.tabIndex(
            index
        )}document.dispatchEvent( event${index} );`;
    };

    scrollTo = (xPosition, yPosition) =>
        `document.documentElement.scrollTo({left: ${xPosition}, top: ${yPosition}, behavior: 'smooth'});`;

    elementScrollTo = (selector, xPosition, yPosition) =>
        `document.querySelector('${selector}').scrollTo({left: ${xPosition}, top: ${yPosition}, behavior: 'smooth'});`;

    focus = (selector) => `document.querySelector('${selector}').focus({ preventScroll: false });`;

    hover = (selector, index) =>
        `const event${index} = new MouseEvent('mouseover', {view: window, bubbles: true, cancelable: false}); ${this.tabIndex(
            index
        )}document.querySelector('${selector}').dispatchEvent( event${index} );`;

    textSelect = (selector, index) =>
        `const range${index} = document.createRange(); ${this.tabIndex(
            index
        )}const referenceNode${index} = document.querySelector('${selector}'); ${this.tabIndex(
            index
        )}range${index}.selectNode(referenceNode${index}); ${this.tabIndex(
            index
        )}const currentSelection${index} = window.getSelection(); ${this.tabIndex(
            index
        )}currentSelection${index}.removeAllRanges(); ${this.tabIndex(
            index
        )}currentSelection${index}.addRange(range${index});`;

    //ASSERTIONS HELPERS

    getTitle = (selector = 'document', index) =>
        selector == 'document'
            ? `const title${index} = document.title; `
            : `const title${index} = document.querySelector('${selector}').title;`;
    ...
    countElements = (selector, index) =>
        `const count${index} = Array.prototype.slice.call(document.querySelectorAll('${selector}')).length;`;

    getElementProperty = (selector, property, index) =>
        `const property${index} = document.querySelector('${selector}').${property};`;

    getElementAttributeValue = (selector, attribute, index) =>
        `const ${attribute}Attribute${index} = document.querySelector('${selector}').getAttribute('${attribute}');`;

    getElementAttributesAsArray = (selector, index) =>
        `const attributesArray${index} = Array.prototype.slice.call(document.querySelector('${selector}').attributes);`;
    ...
    buildRecordingStringFromEvents = (eventsArray) => {
        ...
        for (let recordingEventIndex in eventsArray) {
            ...
            if (recordingEventIndex == 0) {
                ...
                outputString += `${this.tabIndex(recordingEventIndex)}${this.mapActionTypeToFunction(
                    eachEvent,
                    recordingEventIndex
                )}`;
