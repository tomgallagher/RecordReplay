class DomSelectorReport {
    ...
    constructor(options) {
        ...
        return (async () => {
            ...
            const targetId = await new Promise(res => chrome.debugger.sendCommand({ tabId: this.browserTabId }, "DOM.querySelector", { nodeId: doc.nodeId, selector: this.selectorString}, nodeId => res(nodeId) ) );
            ...
            const targetNode = await new Promise(res => chrome.debugger.sendCommand({ tabId: this.browserTabId }, "DOM.describeNode", { nodeId: targetId, depth: -1, pierce: true }, node => res(node) ) );
